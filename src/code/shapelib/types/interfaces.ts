...
export interface Transformable<O extends Transformable<O>> extends BoundingBox {
    offset(offset: Point): O;
    flip(axis: Axis): O;
    rotate(angle: number, origin?: Point): O; // origin is infered as center of bbox
}
...
export interface HasArea<O extends HasArea<O>> extends BoundingBox {
    area(): number;
    ...
    scale(scale: number, origin?: Point): O; // origin is infered as center of bbox
}
...
export interface Shape<O extends Shape<O>>
    extends Transformable<O>,
        Stringifiable,
        HasArea<O>,
        HasLength<O>,
        Renderable {
