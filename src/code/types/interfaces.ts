...
export interface PointMap<O extends PointMap<O>> {
    map_points(f: (p: Point) => Point): O;
}
...
export interface Transformable<O extends Transformable<O>> {
    offset(offset: Point): O;
    scale(scale: number, offset?: Point): O;
    flip(axis: Axis): O;
}
...
export type CanvasStyle = typeof CanvasRenderingContext2D.prototype.fillStyle | typeof CanvasRenderingContext2D.prototype.strokeStyle;

export interface Renderable extends RenderableDebug {
...
export interface HasLength<O extends HasLength<O>> {
    outline_length(): number;
    sample_on_length(min_per_unit: number, variant: "rng" | "evenly"): Point[];
    outline_intersects(other: O): boolean;
    right_point_intersections(p: Point): number;
}


export interface SolidShape<O extends SolidShape<O>> extends Shape<O> {
    ...
    relation_to(other: O): "this_inside_other" | "other_inside_this" | "outline_intersect" | "disjoint"
    intersects(other: O): boolean;
}

export interface Shape<O extends Shape<O>> extends Transformable<O>, Stringifiable, BoundingBox, HasArea, HasLength<O>, Renderable { }
