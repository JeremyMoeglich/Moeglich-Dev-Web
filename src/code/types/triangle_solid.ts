...
export class TriangleSolid implements SolidShape<TriangleSolid>, PointMap<TriangleSolid> {
    a: Point;
    b: Point;
    c: Point;
    ...
    offset(p: Point): TriangleSolid {
        return new TriangleSolid(this.a.offset(p), this.b.offset(p), this.c.offset(p));
    }

    scale(scale: number, offset?: Point): TriangleSolid {
        return new TriangleSolid(this.a.scale(scale, offset), this.b.scale(scale, offset), this.c.scale(scale, offset));
    }

    flip(axis: Axis): TriangleSolid {
        return new TriangleSolid(this.a.flip(axis), this.b.flip(axis), this.c.flip(axis));
    }
    ...
    area(): number {
        return Math.abs(
            ...
            this.c.x * this.a.y - this.c.y * this.a.x
        ) / 2;
    }
    ...
    outline_length(): number {
        return cyclic_pairs(this.vertices()).map(([a, b]) => a.distance(b)).reduce((a, b) => a + b, 0);
    }
    ...
    outline_intersects(other: TriangleSolid): boolean {
        return this.lines().some(l => other.lines().some(l2 => l.outline_intersects(l2)));
    }

    intersects(other: TriangleSolid): boolean {
        return this.contains(other.a) || this.contains(other.b) || this.contains(other.c) || this.outline_intersects(other);
    }

    relation_to(other: TriangleSolid): "this_inside_other" | "other_inside_this" | "outline_intersect" | "disjoint" {
        if (this.contains(other.a) && this.contains(other.b) && this.contains(other.c)) {
    ...
    right_point_intersections(p: Point): number {
        const lines = cyclic_pairs(this.vertices()).map(([a, b]) => new LineSegment(a, b));
        return lines.reduce((acc, l) => acc + l.right_point_intersections(p), 0);
    }
