...
import type { Axis, HasVertices, SolidShape } from "./interfaces";
...
export class RectSolid implements SolidShape<RectSolid>, HasVertices {
    x: number;
    y: number;
    width: number;
    height: number;
    ...
    offset(p: Point): RectSolid {
        return new RectSolid(this.x + p.x, this.y + p.y, this.width, this.height);
    }

    scale(scale: number, offset?: Point): RectSolid {
        const offsetX = offset?.x ?? 0;
        const offsetY = offset?.y ?? 0;
        return new RectSolid(this.x * scale + offsetX, this.y * scale + offsetY, this.width * scale, this.height * scale);
    }

    flip(axis: Axis): RectSolid {
        switch (axis) {
            case 'x':
                return new RectSolid(-this.x - this.width, this.y, this.width, this.height);
            case 'y':
                return new RectSolid(this.x, -this.y - this.height, this.width, this.height);
            case 'both':
                return new RectSolid(-this.x - this.width, -this.y - this.height, this.width, this.height);
        }
    }
    ...
    sample_points_area(n: number): Point[] {
        return range(n).map(_ => new Point(
    ...
    sample_on_length(min_per_unit: number, variant: 'evenly' | 'rng'): Point[] {
        ...
        const lines = cyclic_pairs(this.vertices()).map(([a, b]) => new LineSegment(a, b));
        if (variant === 'rng') {
            return range(n).flatMap(_ => {
                const line = sample(lines) ?? panic("No line to sample from, (this should never happen)");
                return line.sample_points(1, 'rng');
            })
        } else {
            const n_per_line = Math.ceil(n / lines.length);
            return lines.flatMap(line => line.sample_points(n_per_line, 'evenly'));
        }
    }

    triangulate(): TriangleSolid[] {
        return [
            new TriangleSolid(new Point(this.x, this.y), new Point(this.x + this.width, this.y), new Point(this.x + this.width, this.y + this.height)),
            new TriangleSolid(new Point(this.x, this.y), new Point(this.x + this.width, this.y + this.height), new Point(this.x, this.y + this.height)),
        ];
    }
    ...
    relation_to(other: RectSolid): "this_inside_other" | "other_inside_this" | "outline_intersect" | "disjoint" {
        if (this.x >= other.x && this.x + this.width <= other.x + other.width && this.y >= other.y && this.y + this.height <= other.y + other.height) {
            return "this_inside_other";
        } else if (other.x >= this.x && other.x + other.width <= this.x + this.width && other.y >= this.y && other.y + other.height <= this.y + this.height) {
            return "other_inside_this";
        } else if (this.x + this.width < other.x || this.x > other.x + other.width || this.y + this.height < other.y || this.y > other.y + other.height) {
            return "disjoint";
        } else {
    ...
    lines(): LineSegment[] {
        return cyclic_pairs(this.vertices()).map(([a, b]) => new LineSegment(a, b));
    }
    ...
    distribute_grid(min_n: number): Point[] {
        ...
        return range(n_x).flatMap(i => range(n_y).map(j => new Point(this.x + i * dx, this.y + j * dy)));
    }
}