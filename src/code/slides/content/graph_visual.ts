...
function hashNumber(n: number, lower_bound: number, upper_bound: number): number {
...
function generate_point(seed: number, t: number) {
    ...
    const samples = range(0, 20).map(i => {
        ...
        const point = new Point(hashNumber(sample + seed, 0, 1000), hashNumber(-sample + seed, 0, 1000));
    ...
    const total_influence = samples.reduce((acc, [_, influence]) => acc + influence, 0);
    return samples.map(([point, influence]) => {
