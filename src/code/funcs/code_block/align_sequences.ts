...
export function alignSequences(previousSequences: Token[][], newSequences: Token[][]): Array<[KeyedToken | null, Token | null][]> {
    ...
    const previousTokens: Token[] = previousSequences.reduce((a, b) => a.concat(b), []);
    const newTokens: Token[] = newSequences.reduce((a, b) => a.concat(b), []);

    // Initialize Needleman-Wunsch matrix.
    const matrix: number[][] = Array.from({length: previousTokens.length + 1}, () => 
    ...
    for (let i = 1; i <= previousTokens.length; i++) {
        for (let j = 1; j <= newTokens.length; j++) {
            let match = matrix[i - 1][j - 1] + score(previousTokens[i - 1], newTokens[j - 1]);
            let deleteOld = matrix[i - 1][j] - 1;
            let insertNew = matrix[i][j - 1] - 1;
    ...
    while (i > 0 && j > 0) {
        if (matrix[i][j] === matrix[i - 1][j - 1] + score(previousTokens[i - 1], newTokens[j - 1])) {
