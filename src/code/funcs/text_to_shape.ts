...
export async function textToShapes(text: string, fontFilePath = '/fonts/roboto/Roboto-Bold.ttf'): Promise<HollowShape<BezierSolid>[]> {
    ...
    function convert_coords<T extends SolidShape<T>>(shape: HollowShape<T>): HollowShape<T> {
        return shape.offset(new Point(0, -1800)).flip('y');
    }

    const shapes_from_glyph = (glyph: Glyph, offset: number): HollowShape<BezierSolid>[] =>
        parseCommands(glyph.path.commands).map(shape => convert_coords(shape.offset(new Point(offset, 0))));
...
function beziersToShapes(beziers: BezierSolid[]): HollowShape<BezierSolid>[] {
    ...
    const shapes: HollowShape<BezierSolid>[] = [new HollowShape(
        beziers[0] ?? panic(),
        []
    )];
